...
while ($boards = mysqli_fetch_assoc($r)) {
    //Start of list
    echo '<h3 class="text-sm flex-shrink-0 truncate overflow-hidden!important bg-slate-100 py-2 hover:bg-slate-200 cursor-pointer pl-2" onclick="confirmMoving(' . $boards['boardID'] . ', ' . $dataID . ')">' . $boards['board_name'] . '</h3>';
    //End of list
}
echo '      </div>';
echo '  </div>';
