...
if (mysqli_num_rows($result) > 0) {
    while ($projects = mysqli_fetch_assoc($result)) {
        ...
        echo '  <div class="flex h-1 transition-all duration-300 group-hover:h-full w-full absolute ' . $projects['color_code'] . ' group-hover:rounded-lg flex-shrink-0 rounded-tr-lg rounded-tl-lg"></div>';
        ...
        if (strlen($projects['project_description']) > 0) {
            echo '<div class="flex w-52 mx-auto group-hover:z-10 group-hover:text-gray-300 transition text-xs text-gray-500 px-4 pb-8 rounded-lg break-all text-left resize-none overflow-y-hidden" title="' . $projects['project_description'] . '">' . $projects['project_description'] . '</div>';
        } else {
