...
while ($priorities = mysqli_fetch_assoc($result)) {
    //Start of list
    echo '<div class="prioritySaveBtn flex pl-2 py-2 text-sm bg-slate-100 hover:bg-slate-200 cursor-pointer" data-priority-name="' . $priorities['priority_name'] . '">' . $priorities['priority_name'] . '</div>';
    //End of list
}
echo '      </div>';
echo '  </div>';
