...
$q = "select projectID from project where project_name = '$projectName' and userID = '$userID'";
$r = mysqli_fetch_assoc(mysqli_query($conn, $q));
$projectID = $r['projectID'];

$q = "select project.projectID, project.project_description, colors.color_code, colors.color_name from project join colors on project.colorID = colors.colorID where project.projectID = $projectID";
...
echo '          <div class="my-1 ml-4 w-full h-full font-bold select-none">Project edit</div>';
...
echo '              <input placeholder="Type a new project name" id="projectNameEdit" type="text" class="flex shadow-md form-control block w-full px-2 text-base font-normal text-slate-700 bg-slate-50 bg-clip-padding border border-solid border-slate-300 rounded-md transition ease-in-out duration-200 m-0 focus:text-slate-700 focus:bg-slate-50 focus:border-slate-600 focus:outline-none" value="' . $projectName . '"></input>';
echo '          </div>';
echo '          <div class="wrap mt-4 w-full h-full px-6 space-y-1 flex flex-col">';
echo '              <label class="font-bold">Description</label>';
echo '              <textarea id="projectDescriptionEdit" placeholder="Describe this project" maxlength="128" class="descriptionInput flex overflow-y-auto shadow-md h-20 form-control block w-full px-2 text-base font-normal text-slate-700 bg-slate-50 bg-clip-padding border border-solid border-slate-300 rounded-md transition ease-in-out duration-200 m-0 focus:text-slate-700 focus:bg-slate-50 focus:border-slate-600 focus:outline-none resize-none" autocomplete="none">' . $project_description . '</textarea>';
echo '          </div>';
echo '          <div class="wrap mt-4 w-full h-full px-6 space-y-1 flex flex-col">';
...
echo '                      <div id="currentColor" class="flex w-4 h-4 mt-[0.3rem] ml-2 rounded-md ' . $color_code . '"></div>';
echo '                      <div id="currentColorName" class="flex h-4 mt-[0.1rem] pl-3">' . $color_name . '</div>';
...
echo '          <div class="flex">';
echo '              <div title="Delete whole project" class="w-fit transition h-fit px-6 py-2.5 mr-auto bg-red-300 border border-red-600 hover:bg-red-500 rounded-lg cursor-pointer group" onclick="showProjectDeleteWarning(`' . $projectName . '`)"><svg class="w-3 h-3 fill-white transition-all" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--! Font Awesome Pro 6.2.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M135.2 17.7L128 32H32C14.3 32 0 46.3 0 64S14.3 96 32 96H416c17.7 0 32-14.3 32-32s-14.3-32-32-32H320l-7.2-14.3C307.4 6.8 296.3 0 284.2 0H163.8c-12.1 0-23.2 6.8-28.6 17.7zM416 128H32L53.2 467c1.6 25.3 22.6 45 47.9 45H346.9c25.3 0 46.3-19.7 47.9-45L416 128z"/></svg></div>';
echo '          </div>';
...
echo '              <div class="w-fit h-fit px-3 py-1 bg-slate-500 border border-slate-400 hover:bg-slate-600 rounded-lg cursor-pointer text-slate-50" onclick="saveProjectChanges(`' . $projectName . '`)">Save</div>';
echo '          </div>';
echo '      </div>';
echo '  </div>';
echo '</div>';
