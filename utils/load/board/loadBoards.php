...
function loadAddNewBoard($projectID) {
    ...
    echo '<div title="Add a new column" class="newBoardButton flex w-64 transition-[width] ease-in-out text-gray-400 duration-200 text-lg bg-slate-100 hover:bg-slate-200 rounded-md cursor-pointer" onclick="expandBoardCreate()">';
    echo '<span class="mx-auto">+ Add column</span>';
    echo '</div>';
...
if (mysqli_num_rows($result) > 0) {
    while ($boards = mysqli_fetch_assoc($result)) {
        ...
        $q = 'select * from board_data where boardID = ' . $boards['boardID'] . '';
        $total = mysqli_num_rows(mysqli_query($conn, $q));
        $q = 'select * from board_data where boardID = ' . $boards['boardID'] . ' && board_check = 1';
        $totalComplete = mysqli_num_rows(mysqli_query($conn, $q));
        $percentComplete = $totalComplete / $total * 100;
        echo '<div class="flex h-fit w-fit px-2 text-sm my-auto rounded-full bg-slate-200 justify-center">' . $total . '</div>';
        echo '<div class="flex w-fit px-1 py-1 h-fit py-1 mr-1 ml-auto my-1 transition ease-in-out hover:bg-slate-200 cursor-pointer rounded-lg" onclick="showColumnManagePopup(`' . $boards['boardID'] . '`)"><svg class="w-4 px-auto h-4 fill-slate-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.2.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M0 96C0 78.3 14.3 64 32 64H416c17.7 0 32 14.3 32 32s-14.3 32-32 32H32C14.3 128 0 113.7 0 96zM64 256c0-17.7 14.3-32 32-32H480c17.7 0 32 14.3 32 32s-14.3 32-32 32H96c-17.7 0-32-14.3-32-32zM448 416c0 17.7-14.3 32-32 32H32c-17.7 0-32-14.3-32-32s14.3-32 32-32H416c17.7 0 32 14.3 32 32z"/></svg></div>';
        echo '</div>';
        //Progress bar
        echo '<div class="flex flex-row">';
        echo '<div id="prog-bar" class="rounded-full flex w-full h-2 bg-gray-200 my-auto">';
        echo '  <div class="w-[' . $percentComplete . '%] bg-lime-300 rounded-full transition-all animate-pulse"></div>';
        echo '</div>';
        ...
        while ($board_data = mysqli_fetch_array($resultBD)) {
            if ($board_data['board_check'] == 0) {
                ...
                echo '      <div title="More actions" class="flex w-fit px-2 left-[14.5rem] absolute opacity-0 cursor-pointer transition ease-in-out hover:bg-slate-200 rounded-lg group-hover:opacity-100" onclick="showTaskManagePopup(`' . $board_data['dataID'] . '`)"><svg class="w-4 px-auto h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--! Font Awesome Pro 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M120 256c0 30.9-25.1 56-56 56s-56-25.1-56-56s25.1-56 56-56s56 25.1 56 56zm160 0c0 30.9-25.1 56-56 56s-56-25.1-56-56s25.1-56 56-56s56 25.1 56 56zm104 56c-30.9 0-56-25.1-56-56s25.1-56 56-56s56 25.1 56 56s-25.1 56-56 56z"/></svg>';
                echo '  </div>';
                echo ' </div>';
                ...
                echo '  <div class="flex-shrink-0 flex flex-row px-2">';
                echo '      <div title="Mark incomplete" class="flex h-6 w-6 border-2 flex-shrink-0 border-gray-300 rounded-full bg-lime-500 cursor-pointer" onclick="isChecked(`' . $board_data['dataID'] . '`)">';
        ...
        echo '<div class="flex mx-auto my-auto">+ Add task</div>';
        echo '</div>';
        echo '</div>';
        //End of flex area
        echo '</div>';
