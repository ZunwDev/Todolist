...
if (mysqli_num_rows($result) > 0) {
    while ($projects = mysqli_fetch_assoc($result)) {
        ...
        echo '<textarea id="' . $projects['project_name'] . '_edit" type="text" class="form-control px-1 w-1/2 flex h-6 text-md resize-none ml-auto my-auto hidden overflow-auto overflow-y-hidden rounded-md transition ease-in-out focus:text-gray-700 focus:bg-slate-50 focus:border-blue-600 focus:outline-blue-600">' . $projects['project_name'] . '</textarea>';
        //Buttons
        ...
        //Cancel changes
        echo '<button type="button" onclick="cancelChangesRename()" title="Cancel changes - Esc" class="z-10 flex w-6 h-8 hover:bg-red-200 rounded-tr-lg rounded-br-lg">';
