...
if (mysqli_num_rows($result) > 0) {
    while ($projects = mysqli_fetch_assoc($result)) {
        ...
        echo '<div class="flex w-4 h-4 my-auto ml-2 rounded-md z-4 ' . $projects['color_code'] . '" value="' . $projects['color_code'] . '"></div>';
        ...
        //Save changes
        echo '<button type="button" onclick="saveChanges()" title="Save changes" class="z-2 flex w-6 h-8 hover:bg-green-200">';
        ...
        //Cancel changes
        echo '<button type="button" onclick="cancelChanges()" title="Cancel changes" class="z-2 flex w-6 h-8 hover:bg-red-200 rounded-tr-lg rounded-br-lg">';
