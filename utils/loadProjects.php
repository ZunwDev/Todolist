...
if (mysqli_num_rows($result) > 0) {
    while ($projects = mysqli_fetch_assoc($result)) {
        echo '<button type="button" class="flex group gap-2 flex-col w-52 h-fit shadow-xl relative transition rounded-lg bg-slate-100" onclick="openProject(`' . $projects['projectID'] . '`)">';
...
$q = "select projectID from project where userID = '$userID'";
