...
if (mysqli_num_rows($result) > 0) {
    while ($projects = mysqli_fetch_assoc($result)) {
        echo '<button type="button" class="flex flex-col w-40 h-fit transition ease-in-out duration-200 rounded-lg bg-slate-200 hover:bg-slate-300" onclick="openProject(`'.$projects['projectID'].'`, `'.$projects['project_name'].'`, `'.$projects['project_description'].'`, `'.$projects['color_code'].'`)">';
        ...
        if (strlen($projects['project_description']) > 0) {
            echo '<div class="flex w-40 mx-auto text-xs text-gray-500 px-2 pb-2 rounded-lg break-words text-left resize-none overflow-y-hidden" title="' . $projects['project_description'] . '">' . $projects['project_description'] . '</div>';
        } else {
