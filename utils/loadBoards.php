...
if (mysqli_num_rows($result) > 0) {
    while ($boards = mysqli_fetch_assoc($result)) {
        ...
        //Board name 
        echo '<div class="flex w-56 transition ease-in-out duration-200 h-8 text-lg bg-slate-200 hover:bg-slate-300 rounded-md cursor-pointer">';
        echo '<div class="flex px-2 my-auto">'.$boards['board_name'].'</div>';
        echo '</div>';
        //Adding new task
        echo '<div class="flex mt-2 w-56 h-8 bg-slate-200 hover:bg-slate-300 rounded-md cursor-pointer">';
