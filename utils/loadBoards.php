...
if (mysqli_num_rows($result) > 0) {
    while ($boards = mysqli_fetch_assoc($result)) {
        ...
        //Board name 
        echo '<div class="flex w-64 transition ease-in-out duration-200 h-8 text-lg bg-slate-200 hover:bg-slate-300 rounded-tr-md rounded-tl-md cursor-pointer">';
        ...
        while ($board_data = mysqli_fetch_array($resultBD)) {
            if ($board_data['board_check'] == 0) {
                ...
                echo '<div class="h-fit flex-col mt-2 bg-slate-200 hover:bg-slate-300 cursor-pointer group">';
                echo '  <div class="w-64 h-fit flex flex-row break-words">';
                ...
                echo '      <div title="More actions" class="flex w-fit px-2 h-fit py-1 mr-1 my-1 opacity-0 transition ease-in-out hover:bg-slate-400 rounded-lg group-hover:opacity-100" onclick="showTaskManagePopup(`' . $board_data['dataID'] . '`, `' . $project_name . '`)"><svg class="w-4 px-auto h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--! Font Awesome Pro 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M120 256c0 30.9-25.1 56-56 56s-56-25.1-56-56s25.1-56 56-56s56 25.1 56 56zm160 0c0 30.9-25.1 56-56 56s-56-25.1-56-56s25.1-56 56-56s56 25.1 56 56zm104 56c-30.9 0-56-25.1-56-56s25.1-56 56-56s56 25.1 56 56s-25.1 56-56 56z"/></svg>';
                echo '  </div>';
                echo ' </div>';
                ...
                if ($board_data['priority_name'] !== null) {
                ...
                echo '<div class="h-fit flex w-64 flex-row mt-2 bg-lime-300 hover:bg-lime-400 cursor-pointer group">';
                echo '  <div class="h-fit flex break-words">';
                echo '      <div class="flex py-1 px-1">';
        ...
        echo '</div>';
        //Adding new task
        echo '<div class="flex flex-col w-64 gap-2 my-2">';
        ...
        echo '<div id="' . $boards['boardID'] . '_confirm" class="flex mt-2 h-8 bg-lime-400 hover:bg-lime-500 rounded-md cursor-pointer hidden" onclick="saveData(`' . $boards['boardID'] . '`, `' . $boards['project_name'] . '`)">';
